...
celltype-cli is an autonomous agent for drug discovery research — like Codex, but for biology.
...
**Codex Agent SDK agentic loop**: Query → Codex (plans, calls tools, self-corrects, synthesizes) → Report

Uses ClaudeSDKClient with an in-process MCP server exposing all domain tools. Codex orchestrates
